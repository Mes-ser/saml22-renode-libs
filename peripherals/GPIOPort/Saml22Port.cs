...
namespace Antmicro.Renode.Peripherals.GPIOPort
{
    public class Saml22Port : IDoubleWordPeripheral, IBytePeripheral, IKnownSize, IPeripheralRegister<IGPIOReceiver, NumberRegistrationPoint<int>>
    {
        ...
        public void Reset()
        {
            foreach (Saml22PortGroup port in portsGroup.Cast<Saml22PortGroup>())
            {
                port.Reset();
            }
        }

        public uint ReadDoubleWord(long offset) => ((Saml22PortGroup)portsGroup[(int)(offset / GROUP_OFFSET)]).doubleWordRegisters.Read(offset % GROUP_OFFSET);
        public void WriteDoubleWord(long offset, uint value) => ((Saml22PortGroup)portsGroup[(int)(offset / GROUP_OFFSET)]).doubleWordRegisters.Write(offset % GROUP_OFFSET, value);
        public byte ReadByte(long offset) => ((Saml22PortGroup)portsGroup[(int)(offset / GROUP_OFFSET)]).byteRegisters.Read(offset % GROUP_OFFSET);
        public void WriteByte(long offset, byte value) => ((Saml22PortGroup)portsGroup[(int)(offset / GROUP_OFFSET)]).byteRegisters.Write(offset % GROUP_OFFSET, value);

        public void Register(IGPIOReceiver port, NumberRegistrationPoint<int> portNumber)
        {
            machine.RegisterAsAChildOf(this, port, portNumber);
            portsGroup.Add(port);
        ...
        private readonly List<IGPIOReceiver> portsGroup = new List<IGPIOReceiver>();
